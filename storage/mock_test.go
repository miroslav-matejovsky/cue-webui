...
func TestMock_LoadReturnsCopy(t *testing.T) {
	...
	values, err := store.LoadMap(context.Background())
...
func TestMock_SaveReplacesValues(t *testing.T) {
	...
	err := store.SaveMap(context.Background(), map[string]string{"server.port": "8080"})
	require.NoError(t, err)
...
func TestMock_ConfiguredErrors(t *testing.T) {
	...
	_, err := store.LoadMap(context.Background())
	require.ErrorIs(t, err, loadErr)

	store.SetLoadError(nil)
	store.SetSaveError(saveErr)
	err = store.SaveMap(context.Background(), map[string]string{"key": "value"})
